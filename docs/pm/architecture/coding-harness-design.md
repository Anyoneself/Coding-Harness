...
详细需求统一维护在[产品任务清单](../tasks/task.md)中，并拆分为带完成状态和依赖关系的子任务。本文只保留迭代形态：

1. **可持续执行底座：** 已完成。产品从一次性代码聊天演进为可后台运行、重放、中断和恢复的只读任务。
2. **安全交付与双端首发：** 当前主迭代。产品增加多 Provider、受控工具、审批、可信沙箱、ChangeSet、冲突保护和 Verification，并以 Web 工作台与 CLI 双入口首发。
3. **可配置内测产品：** 在安全交付稳定后，增加项目指令、Skill、Provider 治理与路由、自动化集成、可观测性、成本和反馈闭环。

任务按依赖与退出条件推进，不绑定固定日期、开发周期或人员配置。
